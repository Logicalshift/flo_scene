...
impl SceneContext {
    ...
    ///
    /// A command is a type of subprogram that has a direct connection to the subprogram that spawned it. The
    /// spawning subprogram can pass in a stream of input, and the command generates a stream of output. The
    /// command can communicate with the rest of the scene (and can receive messages if needed).
    ///
    /// In addition, a query can be created with `spawn_query`: this is a special type of command where the
    /// input is generated by a third subprogram, and the output returned to the main program.
    ///
        ...
        TCommand: 'static + Command,
    {
        if let (Some(scene_core), Some(program_core)) = (self.scene_core.upgrade(), self.program_core.upgrade()) {
            ...
            let message_stream = InputStream::new(task_program_id, &scene_core, 0);
            let message_core   = message_stream.core();
            ...
            let run_program = async move {
                if let Ok(scene_context) = recv_context.await {
                    command.run(input, message_stream, scene_context).await;
            ...
            let subtask = SceneCore::start_subprogram(&scene_core, task_program_id, run_program, message_core);
    ...
    where
        TCommand: 'static + Command<Message=()>,
    {
        ...
        if let (Some(scene_core), Some(program_core)) = (self.scene_core.upgrade(), self.program_core.upgrade()) {
            ...
            // Queries can't receive any other messages other than the initial response to their request
            let empty_input           = InputStream::<()>::new(task_program_id, &scene_core, 0);
                ...
                if let Ok(scene_context) = recv_context.await {
                    ...
                    if let Ok(()) = target_connection.send(query.with_new_target(task_program_id.into())).await {
                        ...
                        if let Some(response) = response_input_stream.next().await {
                            ...
                            drop(response_input_stream);

                            // Run the command with the response to the query
                            command.run(response, empty_input, scene_context).await;
