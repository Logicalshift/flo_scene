...
impl SceneCore {
        ...
        TSourceMessageType: 'static + SceneMessage,
    {
        ...
        if target_stream_id != filter_output {
            // The filter needs further mapping to match the output stream
            todo!();
        ...
        match target {
            ...
            StreamTarget::Program(target_program_id) => {
                ...
                if let Some(filter) = filter {
                    ...
                    let filtered_input_core = Self::filtered_input_for_program::<TMessageType>(scene_core, *source, filter, target_program_id)?;
