...
#[test]
pub fn connect_two_subprograms_using_filter_then_all() {
    // Scene with two programs that we'll connect together
    let scene           = Scene::default();
    ...
    scene.add_subprogram(program_1, |_: InputStream<()>, context| {
        async move {
            ...
            // Ask the scene control to connect anything using the 'TestMessage' message type to program_2
            // (This should not override the 'correct' connection we already have)
            context.send_message(SceneControl::connect((), program_2, StreamId::with_message_type::<TestMessage>())).await.unwrap();
...
#[test]
pub fn connect_two_subprograms_using_filter_then_all_no_delay_1() {
    // Scene with two programs that we'll connect together
    let scene           = Scene::default();
    let test_program    = SubProgramId::called("test_program");
    let program_1       = SubProgramId::called("program_1");
...
#[test]
pub fn connect_two_subprograms_using_filter_then_all_no_delay_2() {
    // Scene with two programs that we'll connect together
    let scene           = Scene::default();
    let test_program    = SubProgramId::called("test_program");
    let program_1       = SubProgramId::called("program_1");
