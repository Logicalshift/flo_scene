...
fn log_with_long_name() {
    ...
    TestBuilder::new()
        .expect_message_matching(ErrorOutput::Line("\x1b[0;97mI Test program with a lo\x1b[0m | Hello, world\n".into()), "Log message did not match")
        .run_in_scene_with_threads(&scene, test_subprogram, 10);
}
...
fn log_long_error() {
    ...
    TestBuilder::new()
        .expect_message_matching(ErrorOutput::Line("-------------------------+---------------------------------------------------\n\x1b[0;31m! Test program with a lo\x1b[0m | This is a long message that goes on for long enough\n                         |  that it has to wrap, which means this will be prin\n                         | ted on multiple lines: \"Oops\"\n".into()), "Log message did not match")
        .expect_running_scene()
        .run_in_scene_with_threads(&scene, test_subprogram, 10);
}
