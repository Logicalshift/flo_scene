...
mod with_serde_support {
    ...
    fn serialize_deserialize() {
        ...
        json_serializer_filter.into_iter()
            .for_each(|filter|
                { scene.connect_programs((), StreamTarget::Filtered(filter.clone(), serialized_resender), filter.source_stream_id_any().unwrap()).ok(); });
        ...
        json_deserializer_filter.into_iter()
            .for_each(|filter|
                { scene.connect_programs((), StreamTarget::Filtered(filter.clone(), deserialized_receiver), filter.source_stream_id_any().unwrap()).ok(); });
