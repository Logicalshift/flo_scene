use crate::parser::*;
use crate::socket::*;
use crate::commands::command_parse_error::*;
        ...
        match response {
            ...
            CommandResponse::IoStream(create_stream) => {
                ...
                self.stream_json(move |input, output| async move {
                    ...
                    // We leave JSON mode when the output stream is closed (the input stream is interrupted)
                    // We don't care if the input stream is closed early, so we use a custom polling function
