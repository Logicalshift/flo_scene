...
///
/// Runs a binding program: this is a program that will perform the specified action any time the binding changes.
///
/// The binding action is defined with a function specifying which action to perform. It can be updated with a few options, such as
/// whether or not to perform the action immediately or when the scene becomes idle, a 'parent program' that will cause the binding
/// program to stop when the parent stops, and the action to perform when the binding program is shutting down.
///
/// ```
/// # use flo_scene::*;
...
/// # let binding       = bind(42);
/// let action = BindingAction::new(|value, context| async move { println!("New value: {:?}", value); });
/// scene.add_subprogram(program_id, move |input, context| binding_program(input, context, binding, action), 20);
/// ```
///
