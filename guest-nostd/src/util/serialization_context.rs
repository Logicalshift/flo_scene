...
pub trait SerializationContext : Send + Sync {
    ...
    ///
    /// The serialization ID will be correct for receiving on the other side of the connection (ie, will be a `SerializationId::TheirStream`): if you want to reconnect
    /// to it in *this* side of the connection, you will need to call `invert()` to get the stream ID for this side of the connection. This makes the standard use-case
    /// more straightforward - you can just pass this ID straight across the connection and it will work. However, this might need to be accounted for in more complicated 
    /// situations.
