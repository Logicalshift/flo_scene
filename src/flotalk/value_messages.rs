...
lazy_static! {
    ...
    pub static ref TALK_DISPATCH_BOOLEAN: TalkMessageDispatchTable<bool> = TalkMessageDispatchTable::empty()
        ...
        .with_message(*TALK_MSG_OR,                 |val, args, context| if val { TalkContinuation::from(true) } else { args[0].send_message_in_context(TalkMessage::Unary(*TALK_MSG_VALUE), context) })
        ...
        .with_message(*TALK_MSG_IF_FALSE,           |val, args, context| if !val { args[0].send_message_in_context(TalkMessage::Unary(*TALK_MSG_VALUE), context) } else { TalkValue::Nil.into() })
        .with_message(*TALK_MSG_IF_TRUE,            |val, args, context| if val { args[0].send_message_in_context(TalkMessage::Unary(*TALK_MSG_VALUE), context) } else { TalkValue::Nil.into() })
        .with_message(*TALK_MSG_IF_FALSE_IF_TRUE,   |val, args, context| if !val { args[0].send_message_in_context(TalkMessage::Unary(*TALK_MSG_VALUE), context) } else { args[1].send_message_in_context(TalkMessage::Unary(*TALK_MSG_VALUE), context) })
        .with_message(*TALK_MSG_IF_TRUE_IF_FALSE,   |val, args, context| if val { args[0].send_message_in_context(TalkMessage::Unary(*TALK_MSG_VALUE), context) } else { args[1].send_message_in_context(TalkMessage::Unary(*TALK_MSG_VALUE), context) })
