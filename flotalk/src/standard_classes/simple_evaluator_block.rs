    ...
    TalkSymbol: for<'a> From<&'a TSymbol>,
{
    ...
    ///
    fn send_instance_message(&self, message_id: TalkMessageSignatureId, arguments: TalkOwned<SmallVec<[TalkValue; 4]>, &'_ TalkContext>, reference: TalkReference, allocator: &Arc<Mutex<Self::Allocator>>) -> TalkContinuation<'static> {
    ...
    TalkSymbol: for<'a> From<&'a TSymbol>,
{
    ...
    TalkContinuation::do_while(
        ...
        move |_context| talk_evaluate_simple_with_arguments(Arc::clone(&parent_symbol_table), parent_frames.clone(), smallvec![], Arc::clone(&expression)),
        TalkValue::Nil)
        .and_then(move |value| {
